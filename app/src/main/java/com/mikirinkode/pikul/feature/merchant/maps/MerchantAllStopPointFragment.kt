...
import com.mikirinkode.pikul.databinding.FragmentMerchantAllStopPointBinding
...
@AndroidEntryPoint
class MerchantAllStopPointFragment : Fragment(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener,
    GoogleMap.OnMapClickListener, MerchantMainActivity.Companion.TimePickerListener {
    private var _binding: FragmentMerchantAllStopPointBinding? = null
    private val binding get() = _binding!!
    private var dialogBinding: DialogAddNewStopPointBinding? = null

    private val viewModel: MerchantStopPointViewModel by viewModels()
    @Inject
...
//    private var selectedCoordinate: LatLng? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentMerchantAllStopPointBinding.inflate(inflater, container, false)
    ...
    private fun showInputDialog() {
        ...
        builder.setTitle(getString(R.string.txt_dialog_title_add_new_stop_point))
        ...
        dialogBinding?.apply {
            btnAdd.setOnClickListener {
                ...
                if (startTime.isEmpty() ||endTime.isEmpty()){
                    isValid = false
                    Toast.makeText(requireContext(), "Mohon pilih waktu mulai dan selesai", Toast.LENGTH_SHORT).show()
                }

                if (currentSelectedLocation == null){
                    isValid = false
                    Toast.makeText(requireContext(), "Mohon pilih lokasi terlebih dahulu", Toast.LENGTH_SHORT).show()
                }

                if (isValid){
                    val coordinate = "${currentSelectedLocation?.position?.latitude}, ${currentSelectedLocation?.position?.longitude}"
                    viewModel.addStopPoint(name, startTime, endTime, coordinate).observe(viewLifecycleOwner){ result ->
                        // reset data
                        alertDialog.dismiss()
                        ...
                        when (result) {
                            ...
                            is PikulResult.Error -> {
                                ...
                                Toast.makeText(requireContext(), "Gagal menambah data", Toast.LENGTH_SHORT).show()
                            }
                            is PikulResult.Success -> {
                                binding.layoutLoading.visibility = View.GONE
                                Toast.makeText(requireContext(), "Berhasil menambah titik berhenti", Toast.LENGTH_SHORT).show()
                            }
                        }
                    }
                }
    ...
    private fun createMarker(latLng: LatLng): MarkerOptions {
        return MarkerOptions().position(latLng).title("Lokasi Pilihan Anda")
    }
